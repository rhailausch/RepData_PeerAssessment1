...
### The **mean** steps per day is `r format(mean_steps,scientific=FALSE)`. The **median** steps per day is `r format(median_steps,scientific=FALSE)`.
...
### The interval with maximum number of steps (avereged across all days) was interval **`r max`**.
...
```
### There are **`r num_missing`** missing values in the dataset.  
...
### The new **mean** steps per day after imputing is `r format(mean_steps,scientific=FALSE)`. The new **median** steps per day after imputing is `r format(median_steps,scientific=FALSE)`. After imputing the missing data, the mean did not change and the median changed by one step. Imputing the mising values had essentially no impact on the daily step estimates.
